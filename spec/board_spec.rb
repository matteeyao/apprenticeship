...
describe Board do
    ...
    let(:filled_board) do
        filled_board = Board.new(3)

        for row_idx in (0...filled_board.grid.length)
            for col_idx in (0...filled_board.grid[row_idx].length)
                filled_board[[row_idx, col_idx]] = :x
            end
        end
    ...
    let(:empty_four_by_four_board) do
        Board.new(4)
    end

    let(:empty_five_by_five_board) do
        Board.new(5)
    end
    ...
    describe "#initialize" do
        ...
        context "if grid_size 4 is provided" do
            it "sets up an instance variable grid that is four-by-four" do
                test_board = empty_four_by_four_board
                expect(test_board.grid.length).to eq(4)
                test_board.grid.each { |row| expect(row.length).to eq(4)}
                expect(test_board.grid.flatten.length).to eq(16)
            end
        end

        context "if grid_size 5 is provided" do
            it "sets up an instance variable grid that is five-by-five" do
                test_board = empty_five_by_five_board
                expect(test_board.grid.length).to eq(5)
                test_board.grid.each { |row| expect(row.length).to eq(5)}
                expect(test_board.grid.flatten.length).to eq(25)
            end
        end
    end
    ...
    describe "@is_valid?" do
        it "should return false when x or y is less than 0" do
            expect(@empty_board.is_valid?([-1, 1])).to be false
            expect(@empty_board.is_valid?([1, -1])).to be false
        end

        it "should return false when x or y is greater than 2" do
            expect(@empty_board.is_valid?([3, 1])).to be false
            expect(@empty_board.is_valid?([1, 3])).to be false
        end

        it "should return true otherwise" do
            expect(@empty_board.is_valid?([2, 2])).to be true
        end
    end
